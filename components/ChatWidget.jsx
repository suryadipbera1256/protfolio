...
export default function ChatWidget() {
  ...
  const hasDragged = useRef(false); // To prevent clicking when user means to drag
  ...
  const handlePointerMove = (e) => {
    ...
    // If the mouse moves more than a few pixels, we count it as a drag, not a click
  ...
  const toggleOpen = () => {
    if (hasDragged.current) return; // Prevent opening if it was a drag action
    setOpen(!open);
    setShowPopup(false);
  };
      ...
      >
        ...
        {showPopup && !open && (
          <div 
            ...
            className="absolute bottom-[80px] right-0 w-max bg-[#050505]/90 backdrop-blur-2xl border border-cyan-500/30 text-[#f4f4f5] pl-3 pr-6 py-3 rounded-[2rem] rounded-br-md shadow-[0_20px_40px_rgba(0,0,0,0.8),0_0_20px_rgba(6,182,212,0.2)] z-50 flex items-center gap-4 cursor-pointer transition-all duration-500 animate-fade-in-up hover:scale-105 group pointer-events-auto"
          >
        ...
        <div className={clsx(
          "absolute bottom-[80px] right-0 w-[380px] h-[600px] max-w-[calc(100vw-3rem)] max-h-[75vh] z-40 transition-all duration-500 origin-bottom-right flex flex-col pointer-events-auto",
        ...
        {/* --- FLOATING CIRCLE BUTTON (DRAGGABLE) --- */}
        <button
          ...
          className={clsx(
            "absolute bottom-0 right-0 w-16 h-16 rounded-full bg-[#050505] border-2 shadow-[0_0_20px_rgba(6,182,212,0.4)] flex items-center justify-center transition-[background-color,border-color,transform,box-shadow] duration-500 z-50 overflow-hidden group pointer-events-auto",
            open ? "border-[#333] rotate-180 scale-90 shadow-none" : "border-cyan-400 hover:shadow-[0_0_40px_rgba(6,182,212,0.7)] hover:scale-110",
            "cursor-grab active:cursor-grabbing" // Visual indicator for dragging
          )}
        >
